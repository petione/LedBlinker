...
void LedBlinker::setDelay(uint32_t h, uint32_t l,int16_t r) {
    _highDelay = h;
    _lowDelay = l;
    _repeat = r;
}

void LedBlinker::setDelay(uint32_t h, uint32_t l, int16_t r, uint32_t p) {
    ...
    if (diff >= blinkDelay && _repeat != 0)
    {
        ...
        if (_running) {
           ...
           if(_blinkState==0)repeatCounter++;
            if((repeatCounter>=_repeat)&&_repeat>=-1){
                repeatCounter=0;
                _running=false;
